...
| `HW-006` | `[资料复核 + 用户当前状态]` | UART0 板级资料路由与当前使用状态 | 原理图复核记录为 `PA10/U0TX`、`PA11/U0RX` 到 CH340E 网络；**用户确认当前 UART0 未接任何外设/线束，未验证为可用 CH340/COM 链路。** | 原理图 p3；用户现场陈述 |
...
| `PA10/PA11` | UART0 的原理图路由（当前未接线） | CH340E 是资料中的潜在板级路径，不得当作当前激活的外设；复用仍需单独核对路由、Pin Plan 与 SysConfig。 |
...
| `IF-002` | `[阻塞|用户当前状态 + 资料复核]` | `PA10/U0TX`、`PA11/U0RX` 的 UART0 日志候选 | `HW-006` 的原理图路由存在，但当前未接外设/线束、无已验证 COM 链路；不得作为当前日志端口或 DAP UART 接线依据。 | 若未来需要启用，先确认实物板级路由、USB/CH340 枚举、协议、波特率与双驱动风险。 |
