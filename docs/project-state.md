...
- MPU6050 / MPU6000A、nRF24L01+、K230 相关资料处于资料收敛阶段；nRF24L01+只可作为开发期低速遥测/参数调节候选，不能替代题目要求的视频图传。K230 教程确认示例使用 `IO9/IO10` UART 输出目标框 `x/y/w/h`，但教程 MSPM0 对端仅标为 `A22/A21`，不能映射到当前板或替代完整图传链路。
...
- 新增 MS42CG 编码器与 D36A 双路步进驱动资料：MS42CG 的 A/B/PWM/Z 为 0–VCC 输出，D36A 是 STEP/DIR/EN 步进驱动而非电机。实际步进电机型号/相电流、MS42CG 轴向和拓展板取线均未确认。
...
6. D36A 实际步进电机的型号、相电流、绕组线序、MS42CG 安装轴、控制输入电平与供电/散热路径。
